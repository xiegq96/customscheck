$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/drivemonitor/outList',
        datatype: "json",
        colModel: [
            ...
            {   label: '结案单发送次数', name: 'count', index: 'count', width: 120,},
...
var vm = new Vue({
    ...
    methods: {
        ...
        generateList:function () {
            ...
            // var lineId =  $("#jqGrid").jqGrid('getGridParam', 'selrow')
            // var lineData = $("#jqGrid").jqGrid('getRowData',lineId)
            // var lineCount = lineData.count;
            // console.log("lineCount的数值"+lineCount)
            // console.log("下标"+lineId)
            // this.getInfo(id,function () {
                $.ajax({
                    type: "POST",
                    url: baseURL +"sys/drivemonitor/generate",
                    contentType: "application/json",
                    data: JSON.stringify(ids),
                    success: function(r){
                        if(r.code === 0){
                            alert(r.msg, function(index){
                              /* $("#gridTable").setColProp(name,'count','index',lineCount++)
                                console.log("lineCount的数值"+lineCount)*/
                                vm.reload();
